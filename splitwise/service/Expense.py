...
class Expense(ABC):
    ...
    @abstractmethod
    def make_expense(self,payment_from_user_id_list, payment_to_userid,amount,percent_list):
        pass
...
class ExactExpense(Expense):
    ...
    def make_expense(self, payment_from_user_id_list, payment_to_userid, amount_list):
        if not self.__validate_amount(payment_from_user_id_list, amount_list):
            return False
        
        payment_to_user = self.service.get_user(payment_to_userid)

        for idx,payment_from_user_id in enumerate(payment_from_user_id_list):
            payment_from_user = self.service.get_user(payment_from_user_id)
            payment_to_user.set_payment_from(payment_from_user_id, amount_list[idx])
...
class PercentExpense(Expense):
    ...
    def make_expense(self, payment_from_userid_list, payment_to_userid, amount, percent_list):
        ...
        for payment_from_userid in payment_from_userid_list:
            payment_from_user = self.service.get_user(payment_from_userid)
